...
- **Instruction Tokens + Bit Mappings:**  
...
- **IR to Machine Lowering:**  
  - Successfully lowered from IR to machine instructions using PPCI’s backend infrastructure.  
...
- **Evidence:**  
  ...
  - Internal tests: parsing simple arithmetic C functions through AST → IR → Attala assembly.  
  - Noted open design question: floating-point support deferred until scalar stability confirmed.  


## Design Choices  

- **Scalar-First Approach:** Began with scalar instructions only, since they provide the foundation for ISA bring-up and debugging. Floating point and vector ops will build on this once the scalar pipeline is stable.  
